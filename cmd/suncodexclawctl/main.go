...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  suncodexclawctl configure [--account assistant] [--yes]")
}
...
func configure(args []string) {
	...
	for _, group := range groupMissing(inspect.Missing) {
		...
		for _, item := range group.items {
			// If already present in effective config, skip prompting.
...
func askForItem(r *bufio.Reader, item missingItem, useYes bool) (any, bool, error) {
	...
	if useYes {
		...
		// If optional and no recommended, skip.
		if item.Optional && def == "" {
...
func buildMissing(effective map[string]any) []missingItem {
	...
	add := func(item missingItem) {
		if isEmpty(getDotted(effective, item.Key)) {
			missing = append(missing, item)
		}
	}

	// Feishu credentials (secrets)
	...
	add(missingItem{Key: "bot_open_id", Prompt: "Bot open_id (optional; can be auto-detected after first @ mention)", Optional: true, Target: "secrets"})

	// Recommended runtime fields (overlay)
	add(missingItem{Key: "bot_name", Prompt: "Bot display name (recommended)", Recommended: "飞书 Codex 助手", Optional: true, Target: "overlay"})
	add(missingItem{Key: "domain", Prompt: "Domain (feishu/lark/custom https://...)", Recommended: "feishu", Optional: true, Target: "overlay"})
	add(missingItem{Key: "reply_mode", Prompt: "Reply mode", Recommended: "codex", Optional: true, Target: "overlay"})
	add(missingItem{Key: "reply_prefix", Prompt: "Reply prefix", Recommended: "AI 助手：", Optional: true, Target: "overlay"})
	add(missingItem{Key: "require_mention", Prompt: "Require @mention to trigger (true/false)", Recommended: "true", Optional: true, Type: "bool", Target: "overlay"})
	add(missingItem{Key: "require_mention_group_only", Prompt: "Require mention only in group chats (true/false)", Recommended: "true", Optional: true, Type: "bool", Target: "overlay"})
